 ...
 */
	...
	void My_Delay(uint32_t delay) {
	    // 1. Stoppa timern
	    TIM2->CR1 &= ~(1 << 0);  // CEN-biten till 0 för att stoppa timern
	    ...
	    // 5. Vänta tills räknaren når den angivna fördröjningstiden
	    while (TIM2->CNT < delay) {
	        // Vänta tills CNT nåt delay-värdet
	    }

	    // 6.
	    TIM2->CR1 &= ~(1 << 0);  // Stoppa timern efter fördröjningen
...
void TextLCD_Home		(TextLCDType * hlcd)
{
	...
	TextLCD_SendByte(hlcd, 0x02, GPIO_PIN_RESET);  // Skicka home command
	My_Delay(2000);  // Vänta enligt databladet
...
{
	TextLCD_SendByte(hlcd, 0x01, GPIO_PIN_RESET);  // Skicka clear command
	My_Delay(2000);
...
{
    TextLCD_SendByte(hlcd, adr | 0x80, GPIO_PIN_RESET);  // Ställ in adress
...
{
    TextLCD_SendByte(hlcd, c, GPIO_PIN_SET);  // Skicka tecken, RS=1 för data
}
